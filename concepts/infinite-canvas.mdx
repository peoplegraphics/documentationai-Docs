...
description: Why PanelWave treats the screen as a window instead of a page — Scott McCloud's infinite canvas, distance-as-time, and how the story graph becomes the camera's trail.
---
...
container is arbitrary. Scott McCloud's **infinite canvas** (formalized in
*Reinventing Comics*, 2000) proposes treating the screen as a **window**: all
panels of a story live on one continuous plane, and the reader's viewport
travels across it.
...
| Free 2D placement + camera trails | The full **McCloud canvas** — a capability unique to graph-based comics |

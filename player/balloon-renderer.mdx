...
The effective style of every bubble is computed in `SpeechBubblesComponent.resolveEffectiveConfig()` by merging three levels with `mergeBalloonConfig(base, override)` (a deep merge for the nested `tail` and `hideBorder` objects):
...
graph LR
    ...
    C --> B[Per-bubble override<br/>bubble.balloonConfig]
    B --> R[Effective BalloonConfig]
```

1. **Base**: the work-level config (`settings.typography.balloon_config`) or, if absent, `DEFAULT_BALLOON_CONFIG` (`normal` type, `cornerRadius: 0.5`, `maxWidth: 120`, `maxHeight: 80`, `'Ames Italic', sans-serif` at 12 px, 2 px black stroke, white fill, tail enabled at 180°/45 px).
...
3. **Bubble**: the bubble's own `balloonConfig` override wins last.

...
- **Target width per view mode**: panel view uses container width × zoom × `devicePixelRatio`; page view uses the panel placement's fraction of the page width × `devicePixelRatio`; [canvas view](/player/canvas-view#performance) uses placement width × camera zoom × `devicePixelRatio`.
...
- **Out-edge preloading**: in panel view, the player warms the artwork of the current panel's outgoing-edge target panels at the current target width via `PreloadService`, honoring `settings.preload` (`strategy` `none`/`panelsAhead`, `maxConcurrent`) and the network-awareness rules above.

Together with the CMS-generated [responsive size ladder](/cms/assets#automatic-processing) in published manifests, this means an overview or phone reader downloads small WebP rungs while a zoomed-in 4K reader gets the full-resolution encode.

...
Dropped media always keeps its **true aspect ratio**: the first image or video in a panel becomes its background and the panel's proportions on the page adjust to match the media, so artwork never appears stretched. (If an asset predates dimension tracking, its size is measured on the fly at drop time and stored.)

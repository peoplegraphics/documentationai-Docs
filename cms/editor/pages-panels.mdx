...
A "Drop here" placeholder shows where the item will land — and the item really lands **at that position**, including panels dragged in from another page or from **Unassigned**; the canvas renumbers immediately. Moving a page or panel to a **different** chapter or page is possible too, but asks for confirmation first — moving a panel can affect the navigation flow defined in the [graph](/cms/editor/graph). If you prefer clicks over dragging, **Move Up** / **Move Down** in the context menu do single-step reorders.

You can also drag a panel from the tree **onto the canvas** (in page view) to place it on the current page.

<Callout kind="info">
A panel can appear on pages of several [page formats](/cms/works#output) at once. **Delete Panel** on a panel that other pages still use only **detaches** it from the current page — the other formats' pages keep it. Only deleting its last page reference removes the panel itself.
</Callout>
...
Pages also belong to a **page format**: each format the work supports (see [Work Properties → Output](/cms/works#output)) keeps its **own set of pages**, so the phone-shaped and the A4 layout of the same scene are separate pages you design independently. The [format selector](/cms/editor/canvas#canvas-toolbar) above the canvas chooses which format's pages you see and edit.

In the manifest and in exports, pages get **format-scoped ids**: each format has its own letter and counter, so the first format's pages are `pg-A1`, `pg-A2`, …, the second format's `pg-B1`, `pg-B2`, … — every format's page numbering starts at 1.

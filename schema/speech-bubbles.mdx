...
description: Reference for SpeechBubble, BalloonConfig, TailConfig, HideBorderConfig, and BoundingBox — the 9 balloon types and the styling cascade.
---

Speech bubbles carry dialogue, thoughts, and narration inside a panel. Each bubble is a `SpeechBubble` object in a panel's `speechBubbles` array. Bubbles are positioned with normalized coordinates, hold localized text, and are styled through a three-level **balloon configuration cascade** rendered by the player's ComicBalloon SVG engine (see [Balloon Renderer](/player/balloon-renderer)).
...
| `balloonConfig` | `BalloonConfigOverride` | No | Per-bubble style overrides, merged onto character or work-level defaults. |
| `visibleIf` | `JsonLogic` | No | Condition controlling bubble visibility, evaluated against [variables](/schema/variables). |
...
Balloon styling merges three levels, most specific wins:
...
graph LR
    ...
    B --> C["Per-bubble override<br/>speechBubbles[].balloonConfig<br/>(BalloonConfigOverride)"]
```
...
3. **Per-bubble override** — a partial `BalloonConfigOverride` on the bubble itself, merged last.
...
|---|---|---|---|
| `balloonType` | enum | `"normal"` | Visual type of the balloon shape — see the 9 types below. |
...
| `rectangle` | Rectangular balloon (corner radius forced to 0). |
...
A work-level default, a character override, and a bubble that overrides both:
...
The bubble renders as a shout balloon (character), light red (character), 16 px text (bubble), with the work-level tail settings except the character's right curve — and only when the `prefs.speech` variable is truthy.

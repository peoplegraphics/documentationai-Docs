...
| `folderIds` | `Identifier[]` | — | Since schema 1.5: ids of [`assets.folders`](#assetsfolders--authoring-folder-tree-schema-15) entries this asset is filed under (n:m, unique) |
...
Optional array of `AssetFolder` objects mirroring the authoring tool's asset-library folder structure, so it survives transfers between authoring systems (e.g. the CMS's [work archives](/cms/export#archive-export)). **Rendering consumers may ignore this block** — it carries no presentation semantics.

| Property | Type | Required | Description |
|----------|------|:--:|-------------|
| `id` | [`Identifier`](/schema/manifest-structure#identifier) | Yes | Folder id referenced by asset `folderIds` |

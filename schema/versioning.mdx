...
```json
{
  "panelwave": {
    ...
    "generator": "panelwave-sample-gen/1.3"
  }
}
```
...
This is why a `1.3.0` manifest still points its `schema` URI at the `1.0/` path — the 1.1/1.2/1.3 additions were merged into the same schema file, and every valid 1.0 manifest remains valid against it.
...
<Callout kind="alert">
The CLI's upgrade target is currently `1.0.0` — it has not yet been synchronized with the 1.1–1.3 format releases (its bundled *validation* schema is current, though). Since 1.1/1.2/1.3 are purely additive or relaxing, older manifests need no migration to be read by current consumers; run `--dry-run` first and review the reported steps before overwriting files.
</Callout>

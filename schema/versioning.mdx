...
```json
{
  "panelwave": {
    ...
    "generator": "panelwave-sample-gen/1.5"
  }
}
```
...
This is why a `1.5.0` manifest still points its `schema` URI at the `1.0/` path — the 1.1/1.2/1.3/1.4/1.5 additions were merged into the same schema file, and every valid 1.0 manifest remains valid against it.
...
All additive; existing manifests remain valid unchanged. 1.5 adds **authoring metadata** blocks so a work can round-trip between authoring systems (they power the CMS's [work archive transfer](/cms/export#archive-export)) — rendering consumers may ignore all of them:
...
Rendered text in the manifest stays in [`LocalizedString`](/schema/manifest-structure) objects as before — the `localization` block does not replace them; it preserves the *workflow* state (stale markers, machine-translation flags) that authoring tools need and players do not.

Details: [Assets](/schema/assets), [Localization concepts](/concepts/localization).
...
<Callout kind="info">
The CLI's upgrade target is `1.5.0` (synchronized with the current format release). Since 1.1–1.5 are purely additive or relaxing, older manifests need no migration to be read by current consumers — upgrading normalizes the header and legacy fields. Run `--dry-run` first and review the reported steps before overwriting files.
</Callout>
